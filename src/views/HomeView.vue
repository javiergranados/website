<script setup lang="ts">
import { useTheme } from '@/composables/useTheme'
import { computed } from 'vue'

const { theme } = useTheme()

const coverSrc = computed(() => `cover-${theme.value}.svg`)
const coverMdSrc = computed(() => `cover-md-${theme.value}.svg`)
const coverLgSrc = computed(() => `cover-lg-${theme.value}.svg`)
</script>

<template>
  <section class="mx-auto w-full max-w-5xl px-4 py-12 md:px-8 md:py-20">
    <div class="flex flex-col items-center gap-8 md:flex-row md:items-center md:gap-12">
      <!-- Cover SVG -->
      <div class="shrink-0">
        <img :src="coverSrc" alt="Javier Granados" class="h-[180px] w-[248px] md:hidden" />
        <img :src="coverMdSrc" alt="Javier Granados" class="hidden h-[280px] w-[520px] md:block lg:hidden" />
        <img :src="coverLgSrc" alt="Javier Granados" class="hidden h-[300px] w-[745px] lg:block" />
      </div>

      <!-- Text content -->
      <div class="flex flex-col items-center gap-6 text-center md:items-start md:text-left">
        <div>
          <p class="mb-2 text-sm font-medium uppercase tracking-widest text-primary/60">Based in Seville, Spain 🇪🇸</p>
          <h1 class="text-3xl font-bold text-primary md:text-4xl">Front-end Developer & Tech Lead</h1>
        </div>

        <p class="max-w-md text-base text-primary/70">
          Crafting polished interfaces with attention to every detail. Focused on frontend, passionate about design.
        </p>

        <!-- CTAs -->
        <div class="flex flex-wrap items-center justify-center gap-3 md:justify-start">
          <router-link
            to="/projects"
            class="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-base-100 transition-all hover:opacity-90 hover:shadow-lg"
            >View Projects</router-link
          >
          <a
            href="mailto:hola@javiergranados.dev"
            class="rounded-lg border border-primary px-5 py-2.5 text-sm font-semibold text-primary transition-all hover:bg-primary/10"
            >Contact me</a
          >
        </div>

        <!-- Social links -->
        <div class="flex items-center gap-4">
          <a
            href="mailto:hola@javiergranados.dev"
            class="fill-primary/60 transition-colors hover:fill-primary"
            aria-label="Email"
          >
            <svg width="20" height="20" viewBox="0 0 20 18">
              <path
                d="M16.999,4.975L16.999,4.975C16.999,4.975,16.999,4.975,16.999,4.975c-0.419-0.4-0.979-0.654-1.604-0.654H4.606c-0.584,0-1.104,0.236-1.514,0.593C3.076,4.928,3.05,4.925,3.037,4.943C3.034,4.945,3.035,4.95,3.032,4.953C2.574,5.379,2.276,5.975,2.276,6.649v6.702c0,1.285,1.045,2.329,2.33,2.329h10.79c1.285,0,2.328-1.044,2.328-2.329V6.649C17.724,5.989,17.441,5.399,16.999,4.975z M15.396,5.356c0.098,0,0.183,0.035,0.273,0.055l-5.668,4.735L4.382,5.401c0.075-0.014,0.145-0.045,0.224-0.045H15.396z M16.688,13.351c0,0.712-0.581,1.294-1.293,1.294H4.606c-0.714,0-1.294-0.582-1.294-1.294V6.649c0-0.235,0.081-0.445,0.192-0.636l6.162,5.205c0.096,0.081,0.215,0.122,0.334,0.122c0.118,0,0.235-0.041,0.333-0.12l6.189-5.171c0.099,0.181,0.168,0.38,0.168,0.6V13.351z"
              />
            </svg>
          </a>
          <a
            href="https://www.linkedin.com/in/javiergranadosdev/"
            target="_blank"
            rel="noreferrer"
            class="fill-primary/60 transition-colors hover:fill-primary"
            aria-label="LinkedIn"
          >
            <svg width="20" height="20" viewBox="0 0 455 455">
              <path
                d="M455 0V455H0V0H455zM307.256 170.139C271.386 170.139 255.373 189.879 246.4 203.686V174.907H178.863C179.768 193.96 178.863 378.003 178.863 378.003H246.4V264.577C246.4 258.491 246.864 252.458 248.64 248.118 253.526 235.968 264.617 223.425 283.281 223.425 307.72 223.425 317.476 242.054 317.476 269.345V378.003H385V261.537C385 199.159 351.693 170.139 307.256 170.139zM141.522 174.906H74.016V378.002H141.522V174.906zM108.211 76.998C85.107 76.998 70 92.157 70 112.085 70 131.585 84.678 147.186 107.323 147.186H107.769C131.32 147.186 145.963 131.585 145.963 112.085 145.521 92.157 131.32 76.998 108.211 76.998z"
              />
            </svg>
          </a>
          <a
            href="https://github.com/javiergranados"
            target="_blank"
            rel="noreferrer"
            class="fill-primary/60 transition-colors hover:fill-primary"
            aria-label="GitHub"
          >
            <svg height="20" width="20" viewBox="0 0 25 25">
              <path
                d="M12 .297c-6.63.0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577.0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93.0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176.0.0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22.0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22.0 1.606-.015 2.896-.015 3.286.0.315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"
              />
            </svg>
          </a>
        </div>
      </div>
    </div>
  </section>
</template>
